const fr: Record<string, any> = {
  header: {
    brand: "ARCH_OS // JDGR_PORTFOLIO",
    status: "STATUT_SYSTÈME: ACTIF",
  },
  sidebar: {
    name: "JUAN DIEGO",
    version: "V_4.0.2_STABLE",
    infrastructure: "ACCUEIL",
    schematics: "COMPÉTENCES",
    logs: "EXPÉRIENCE",
    coreInit: "FORMATION",
    connectNodes: "CONNECTER_NŒUDS",
  },
  footer: {
    copyright: "© 2025 JDGR_ARCH_OFFICE // KERNEL_VER: 8.4.1-LTS",
    github: "REPO_GITHUB",
    docs: "DOCS_STDLIB",
    debug: "MODE_DEBUG",
  },
  hero: {
    ref: "REF_001 // AMORCE_SYSTÈME",
    title1: "Développement de",
    title2: "Systèmes",
    title3: "Évolutifs",
    description: "Ingénieur des Systèmes Informatiques et Développeur FullStack spécialisé en Architecture de Systèmes modulaires et backends haute performance. Construisant une infrastructure numérique robuste et des interfaces immersives depuis init().",
    btnDeploy: "Exécuter Déploiement",
    btnLogs: "Voir Journaux",
  },
  statusCard: {
    title: "MÉTRIQUES_SYSTÈME",
    online: "EN_LIGNE",
    uptime: "TEMPS_ACTIVITÉ:",
    uptimeVal: "2+ ANS",
    arch: "ARCHITECTURE:",
    archVal: "HEXAGONALE / DDD",
    role: "RÔLE:",
    roleVal: "CO-FOUNDER",
    queryOpt: "OPT_REQUÊTES:",
    queryOptVal: "-40%",
    deploy: "DÉPLOIEMENT:",
    deployVal: "DOCKER / CI-CD",
  },
  statsCard: {
    title: "RAPIDES",
    experience: { label: "EXPÉRIENCE", value: "2+ ANS" },
    projects: { label: "PROJETS", value: "5+ LIVRÉS" },
    stack: { label: "STACK PRINCIPAL", value: "JAVA / TS / SPRING / NEST" },
    languages: { label: "LANGUES", value: "ES (NATIF) / EN (B1)" },
  },
  logs: {
    header: {
      command: "> EXÉCUTER: CONSULTER_EXPÉRIENCE",
      title1: "EXPÉRIENCE_",
      title2: "PROFESSIONNELLE",
      description: "HISTORIQUE PROFESSIONNEL ET PROJETS EN VEDETTE. EXPÉRIENCE AVÉRÉE EN DÉVELOPPEMENT FULLSTACK ET ARCHITECTURE LOGICIELLE. STATUT: NOMINAL.",
    },
    statusStable: "STATUT: STABLE",
    status: "STATUT:",
    work: {
      ref1: "REF_ID: 001 // STARTUP",
      title1: "Solvegrades Technology",
      sub1: "SYSTÈME_ERP // MICROSERVICES",
      desc1: "J'ai dirigé la conception et le développement de zéro d'un système ERP pour la gestion logistique et commerciale. Implémentation de l'Architecture Hexagonale et de Microservices pour isoler la logique métier. Optimisation des bases de données réduisant les temps de requête de 40 % avec synchronisation en temps réel via WebSockets. Développement d'un contrôle d'accès basé sur les rôles (RBAC) et intégration de l'API SUNAT pour la validation automatique du RUC des contribuables.",
      tags1: ["NestJS", "Angular", "Microservices", "Hexagonal", "WebSockets"],
      metrics1: [
        { label: "OPT_REQUÊTES:", value: "-40%" },
        { label: "SYNC:", value: "TEMPS RÉEL" },
        { label: "RÔLE:", value: "CO-FOUNDER" },
      ],
      ref2: "REF_ID: 002 // STAGE",
      title2: "GC Negocios Generales",
      sub2: "SYSTÈME_WEB // OPTIMISATION_REACT",
      desc2: "Développement d'un système web complet pour la gestion des produits et l'automatisation du contrôle des stocks. Amélioration de l'acquisition de clients en optimisant le temps de chargement de la plateforme de 25 % grâce à la refactorisation des composants React et à l'optimisation du rendu côté client.",
      tags2: ["React", "Gestion d'État", "Optimisation DOM"],
      ref3: "REF_ID: 003 // BÉNÉVOLAT",
      title3: "AESI",
      sub3: "PLATEFORME_WEB // ÉQUIPE_AGILE",
      desc3: "Collaboration au sein d'une équipe multidisciplinaire agile pour concevoir et livrer le frontend d'une plateforme de bénévolat d'entreprise. Implémentation des flux d'utilisateurs clés pour une inscription et une navigation fluides, garantissant une architecture frontend hautement maintenable.",
      tags3: ["Next.js", "Architecture UI", "Agile"],
    },
    projects: {
      sectionRef: "REF_ID: 004 // OPEN_SOURCE",
      sectionTitle: "Projets Personnels",
      sectionSub: "DÉPÔTS_EXTERNES // GITHUB",

      ref1: "REF_ID: 005 // SYSTÈME_ENTREPRISE",
      title1: "PGT Warehouse System",
      sub1: "SPRING_CLOUD // ANGULAR // KARDEX",
      desc1: "Architecture et développement d'un système d'entreprise complet pour la gestion des stocks (Kardex) et des itinéraires de picking. Le backend est un écosystème Spring Boot robuste orchestré avec Eureka Discovery, Config Server et API Gateway, appliquant strictement l'Architecture Hexagonale. Le frontend modulaire en Angular implémente un contrôle d'accès basé sur les rôles (RBAC), des intercepteurs JWT et la gestion des flux logistiques en temps réel.",
      tags1: ["Spring Cloud", "Angular", "Arch. Hexagonale", "Microservices"],
      metrics1: [
        { label: "ÉTOILES:", value: "—" },
        { label: "FORKS:", value: "—" },
        { label: "STATUT:", value: "STABLE" },
      ],

      ref2: "REF_ID: 006 // E-COMMERCE",
      title2: "Payflow Platform",
      sub2: "NEXT.JS // MICROSERVICES // API_GATEWAY",
      desc2: "Développement d'une plateforme e-commerce full-stack distribuée en microservices. Le backend implémente la Conception Pilotée par le Domaine (DDD) et l'Architecture Hexagonale en Node.js, orchestré via une API Gateway pour le routage centralisé. Le frontend a été construit avec Next.js et Tailwind CSS, gérant de manière sécurisée les flux complexes de panier d'achat et de paiement.",
      tags2: ["Next.js", "Node.js", "Microservices", "SQL Server"],

      ref3: "REF_ID: 007 // MOBILE_APP",
      title3: "Flare Social App",
      sub3: "KOTLIN // CLEAN_ARCH // COMPOSE",
      desc3: "Développement d'une application mobile native de réseau social pour Android. Conçue en suivant strictement les principes de la Clean Architecture (couches Présentation, Domaine et Données) et le modèle MVVM avec Jetpack Compose. Implémente une persistance locale robuste avec Room Database et la gestion des médias via Cloudinary.",
      tags3: ["Kotlin", "Clean Architecture", "Jetpack Compose", "Room DB"],
    },
    research: {
      sectionRef: "REF_ID: 009 // RECHERCHE",
      sectionTitle: "Recherche",
      sectionSub: "RECHERCHE // ACADÉMIQUE",
      ref: "REF_ID: 009 // RECHERCHE",
      title: "Recherche Académique",
      sub: "RECHERCHE // SCALABILITÉ_ERP",
      desc: "Auteur principal d'une recherche sur l'impact des architectures logicielles sur la scalabilité des systèmes ERP, basée sur la comparaison expérimentale de différentes approches architecturales et leur effet sur la performance et la maintenabilité à long terme.",
      tags: ["Recherche", "Architecture Logicielle", "ERP", "Scalabilité"],
    },
  },
  schematics: {
    header: {
      title: "Compétences Techniques",
      ref: "RÉF_SYSTÈME: MATRICE_COMPÉTENCES_JDGR // ENV: PRODUCTION",
    },
    coreInfra: {
      title: "INFRASTRUCTURE_BASE",
      tier: "NIVEAU_ARCH_1",
      card1Title: "Architecture Hexagonale",
      card1Desc: "Patron Ports et Adaptateurs implémentant un isolement strict entre la logique métier et les systèmes externes.",
      card2Title: "Microservices & DDD",
      card2Desc: "Services découplés basés sur la Conception Pilotée par le Domaine, optimisant l'échelle et l'isolement des pannes.",
      card3Title: "API Gateway & Cloud",
      card3Desc: "Routage centralisé, découverte de services (Eureka) et équilibrage de charge pour les écosystèmes distribués.",
    },
    stackV1: {
      title: "STACK_V1",
      skills: [
        { name: "NestJS [Node.js]", level: 95 },
        { name: "Spring Boot [Java]", level: 88 },
        { name: "Angular [TS]", level: 90 },
        { name: "Next.js [React]", level: 85 },
        { name: "Kotlin [Android]", level: 80 },
        { name: "Tailwind CSS v4", level: 92 },
      ],
    },
    databaseNodes: {
      title: "NŒUDS_BASE_DONNÉES",
      node1Title: "PostgreSQL / MySQL",
      node1Sub: "SGBDR // CLUSTERS_PRINCIPAUX",
      node2Title: "SQL Server / Supabase",
      node2Sub: "DONNÉES_ENTREPRISE // BAAS_CLOUD",
      online: "EN_LIGNE",
    },
    toolChain: {
      title: "EXÉCUTION_CHAÎNE_OUTILS",
      cmd1: "docker pull jdgr/workspace:latest",
      out1: "Statut: L'image est à jour pour jdgr/workspace:latest",
      cmd2: "git status",
      out2: "Sur la branche master. Arbre de travail propre.",
      cmd3: "sys.init --tools",
      tools: ["DOCKER", "GIT", "PIPELINES CI/CD", "SPRING CLOUD"],
    },
  },
  core: {
    header: {
      title: "Formation et Contact",
      module: "MODULE: FORMATION // PROTOCOLE_CONNEXION",
    },
    education: {
      title: "Architecture des Connaissances",
      dataLoaded: "[DONNÉES_CHARGÉES]",
      uni1: "Universidad Privada del Norte (UPN)",
      degree1: "DIPLÔME: Ingénierie des Systèmes Informatiques",
      cycle: "CYCLE_EXÉCUTION: 09",
      percentage: "90%",
      uni2: "Universidad Nacional de Ingeniería (UNI)",
      cert2: "CERTIFICATION: SQL Server – Base de Données II + IA (Nov-Déc 2025)",
      status2: "> STATUT: VÉRIFIÉ_VALIDE",
      ref: "REF_EDU_01 // SYS_ARCH",
    },
    blueprint: {
      label: "RECONNAISSANCE_PATRONS_ACTIVE",
    },
    terminalForm: {
      title: "SYSTEM_INPUT.EXE",
      prompt1: "> INITIALISER_PROTOCOLE_COMMUNICATION",
      prompt2: "> ATTENTE_SAISIE_UTILISATEUR...",
      nameLabel: "IDENTIFIANT [Nom]",
      namePlaceholder: "Entrez texte...",
      emailLabel: "ADRESSE_RÉPONSE [Email]",
      emailPlaceholder: "utilisateur@domaine.com",
      msgLabel: "CHARGE [Message]",
      msgPlaceholder: "Écrivez paramètres...",
      submit: "Exécuter Connexion",
    },
    externalNodes: {
      title: "NŒUDS_EXTERNES // RÉSEAU",
      linkedin: "LinkedIn",
      linkedinSub: "GRAPHE_PROFESSIONNEL",
      github: "GitHub",
      githubSub: "DÉPÔT_CODE",
    },
  },
  projectDetail: {
    back: "RETOUR_AUX_JOURNAUX",
    objectives: "OBJECTIFS_PRINCIPAUX",
    challenges: "DÉFIS_TECHNIQUES",
    metrics: "MÉTRIQUES_PROJET",
    resources: "RESSOURCES",
    sourceCode: "CODE_SOURCE",
    liveDemo: "DÉMO_EN_LIGNE",
    statusProduction: "STATUT: PRODUCTION",
    statusStable: "STATUT: STABLE",
    healthNominal: "OPÉRATIONS_NOMINALES",
    healthStatus: "ÉTAT_DE_SANTÉ",
    terminalRecv: "RÉCÉPTEUR_TERMINAL",
    cpuUsage: "UTILISATION_CPU",
    systemSpec: "SPÉC_SYSTÈME",
    securityProtocol: "PROTOCOLE_SÉCURITÉ",
    deployment: "DÉPLOIEMENT",
    uptime: "TEMPS_ACTIVITÉ",
    stack: "PILE_TECH",
    arch: "ARCHITECTURE",
    containers: "CONTENEURS",
    topology: "Visualisation de la Topologie d'Infrastructures Core",
    projects: {
      "payflow-platform": {
        heroDescription:
          "Plateforme e-commerce haute disponibilité conçue pour les transactions à haut volume. Système distribué en microservices avec communication asynchrone via messagerie, persistance transactionnelle et frontend SSR optimisé pour la conversion.",
        deployment: "PRODUCTION // V.1.0",
        objectives: [
          {
            id: "01",
            title: "Flux de Paiement Sécurisé",
            description:
              "Implémentation d'un pipeline de transactions chiffré de bout en bout avec validation asynchrone pour garantir l'intégrité financière à chaque opération.",
          },
          {
            id: "02",
            title: "Panier d'Achat en Temps Réel",
            description:
              "Gestion de l'état global du panier avec synchronisation entre onglets et persistance serveur, supportant des milliers de produits concurrents.",
          },
          {
            id: "03",
            title: "Architecture Microservices",
            description:
              "Services découplés basés sur le DDD avec communication asynchrone via files de messages, permettant une mise à l'échelle indépendante par domaine.",
          },
          {
            id: "04",
            title: "Frontend SSR Optimisé",
            description:
              "Server-Side Rendering avec Next.js pour le SEO et la performance, combiné à Tailwind CSS pour une interface réactive et rapide.",
          },
        ],
        challenges: [
          {
            id: "01",
            title: "Consistance Transactionnelle Distribuée",
            description:
              "Gestion des transactions traversant plusieurs services de paiement et d'inventaire. Implémentation de patterns Saga et compensation pour assurer l'atomicité sans couplage fort.",
          },
          {
            id: "02",
            title: "Routage API Gateway",
            description:
              "Conception d'un gateway centralisé gérant l'authentification, le rate limiting et l'équilibrage de charge entre plus de 8 microservices avec une latence sub-50ms.",
          },
          {
            id: "03",
            title: "Synchronisation d'Inventaire",
            description:
              "Maintien de la consistance du stock en temps réel lors d'événements haute concurrence. Implémentation de locking optimiste et cache distribué.",
          },
        ],
        metrics: {
          uptime: "99.99%",
          stack: "Next.js / Node.js / SQL",
          arch: "Hexagonale / DDD",
          containers: "8 Microservices",
        },
        terminalLines: [
          "> INIT_PAYFLOW_SERVICES... [OK]",
          "> CONNECTING_API_GATEWAY... [OK]",
          "> POLLING_MICROSERVICES...",
          "> SYNC_INVENTORY_STATE... [DONE]",
          "> PAYMENT_PIPELINE_READY",
          "> LOAD_BALANCER: 8 NODES ACTIVE",
          "> PAYFLOW_ECOMMERCE_V1_LOADED.",
        ],
      },
      "pgt-warehouse-system": {
        heroDescription:
          "Système d'entreprise haute disponibilité conçu pour la gestion logistique et d'entrepôt à grande échelle. Écosystème Spring Cloud avec orchestration des services, persistance transactionnelle et frontend modulaire pour les opérations d'inventaire en temps réel.",
        deployment: "STABLE // V.2.1",
        objectives: [
          {
            id: "01",
            title: "Gestion d'Inventaire Kardex",
            description:
              "Système complet de contrôle d'inventaire avec traçabilité des entrées, sorties et mouvements internes. Calcul automatique des coûts moyens et rapports de rotation de stock.",
          },
          {
            id: "02",
            title: "Itinéraires de Picking Optimisés",
            description:
              "Algorithmes d'optimisation des itinéraires de picking réduisant le temps de préparation des commandes dans les entrepôts haute complexité.",
          },
          {
            id: "03",
            title: "Sécurité Basée sur les Rôles",
            description:
              "RBAC complet avec intercepteurs JWT dans Angular et validation des permissions dans chaque microservice. Contrôle d'accès granulaire par module et opération.",
          },
          {
            id: "04",
            title: "Orchestration Spring Cloud",
            description:
              "Écosystème orchestré avec Eureka Discovery pour l'enregistrement des services, Config Server pour la configuration centralisée et API Gateway pour le routage.",
          },
        ],
        challenges: [
          {
            id: "01",
            title: "Écosystème Microservices Spring",
            description:
              "Configuration et orchestration de plus de 6 microservices avec Spring Cloud, incluant discovery, config centralisée, gateway et circuit breakers pour la résilience.",
          },
          {
            id: "02",
            title: "Consistance d'Inventaire Distribuée",
            description:
              "Maintien de la consistance ACID dans les opérations d'inventaire traversant plusieurs services. Implémentation de transactions distribuées et verrouillage pessimiste.",
          },
          {
            id: "03",
            title: "Intégration Frontend-Backend Modulaire",
            description:
              "Communication efficace entre le frontend Angular modulaire et le backend microservices, avec gestion des erreurs, retry automatique et cache de données statiques.",
          },
        ],
        metrics: {
          uptime: "99.95%",
          stack: "Spring Boot / Angular / SQL",
          arch: "Hexagonale / Microservices",
          containers: "6 Clusters Actifs",
        },
        terminalLines: [
          "> INIT_PGT_SERVICES... [OK]",
          "> EUREKA_DISCOVERY: REGISTERED... [OK]",
          "> CONFIG_SERVER: LOADED... [OK]",
          "> API_GATEWAY: ROUTING...",
          "> SPRING_CLOUD_V2_LOADED.",
          "> HEXAGONAL_ARCH: STRICT_MODE",
          "> SYSTEM_READY.",
        ],
      },
      "flare-social-app": {
        heroDescription:
          "Application mobile native de réseau social pour Android avec architecture propre et persistance hybride. Conçue pour la scalabilité et la maintenabilité avec séparation strate des couches, pattern MVVM et composition déclarative de l'UI.",
        deployment: "STABLE // V.1.0",
        objectives: [
          {
            id: "01",
            title: "Clean Architecture Stricte",
            description:
              "Implémentation des trois couches fondamentales (Présentation, Domaine, Données) avec dépendances unidirectionnelles et utilisation d'interfaces pour le découplage.",
          },
          {
            id: "02",
            title: "UI Déclarative avec Compose",
            description:
              "Interface construite entièrement avec Jetpack Compose, exploitant la recomposition, le state hoisting et les animations pour une expérience native fluide.",
          },
          {
            id: "03",
            title: "Persistance Hybride",
            description:
              "Room Database pour le cache local avec synchronisation intelligente, combiné à Cloudinary pour la gestion et l'optimisation des médias en nuage.",
          },
          {
            id: "04",
            title: "Pattern MVVM",
            description:
              "ViewModels réactifs avec Kotlin Coroutines et Flow, gérant l'état de l'UI de manière prévisible et testable.",
          },
        ],
        challenges: [
          {
            id: "01",
            title: "Synchronisation Hors-Ligne En-Ligne",
            description:
              "Implémentation de stratégies de cache avec Room Database permettant une utilisation complète de l'app hors ligne, avec synchronisation automatique lors de la récupération de la connectivité.",
          },
          {
            id: "02",
            title: "Gestion des Médias en Nuage",
            description:
              "Intégration avec Cloudinary pour l'upload, la transformation et la livraison optimisée d'images et vidéos, avec compression adaptative selon le type de réseau.",
          },
          {
            id: "03",
            title: "Architecture de Navigation",
            description:
              "Conception d'un système de navigation complexe avec Compose Navigation supportant les routes imbriquées, le deep linking et les transitions animées entre écrans.",
          },
        ],
        metrics: {
          uptime: "99.90%",
          stack: "Kotlin / Compose / Room",
          arch: "Clean Arch / MVVM",
          containers: "N/A (Natif)",
        },
        terminalLines: [
          "> INIT_FLARE_APP... [OK]",
          "> ROOM_DB: CONNECTED... [OK]",
          "> CLOUDINARY: MEDIA_READY... [OK]",
          "> FIREBASE_AUTH: INITIALIZED",
          "> COMPOSE_UI: RECOMPOSITION_READY",
          "> CLEAN_ARCH: LAYERS_VALIDATED",
          "> FLARE_SOCIAL_V1_LOADED.",
        ],
      },
    },
  },
  language: {
    label: "LANGUE:",
    es: "ES",
    en: "EN",
    fr: "FR",
  },
};

export default fr;